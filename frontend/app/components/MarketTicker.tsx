...
export default function MarketTicker() {
  return (
    <section className="border-b border-[var(--mg-border)] bg-[var(--mg-surface)]">
      <div className="mx-auto flex max-w-[var(--mg-container)] gap-3 overflow-x-auto px-4 py-3 sm:px-6 lg:px-8">
        {indices.map((item) => {
          ...
          return (
            <div
              key={item.name}
              className="min-w-[178px] rounded-2xl border border-[var(--mg-border)] bg-[var(--mg-surface-soft)] px-4 py-3"
            >
              ...
              <div
                className={`mt-1 inline-flex items-center gap-1 text-xs font-medium ${
                  item.up ? "text-[var(--mg-positive)]" : "text-[var(--mg-negative)]"
                }`}
              >
