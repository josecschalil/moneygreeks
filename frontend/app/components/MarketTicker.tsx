...
export default async function MarketTicker() {
  let indices = FALLBACK_INDICES;

  try {
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/indian-indices/`, { next: { revalidate: 60 } });
    if (res.ok) {
      const data = await res.json();
      const items = Array.isArray(data) ? data : (data.results || []);
      if (items.length > 0) {
        indices = items.slice(0, 6).map((item: any) => {
    ...
    console.warn("Could not fetch indian-indices, using fallback data.");
  }
